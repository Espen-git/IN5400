...
class ImageCaptionModel(nn.Module):
    def __init__(self, config: dict):
        ...
        self.attention_layer = nn.Sequential(
            nn.Dropout(0.25),
            nn.Linear((self.nn_map_size*2), 50),
            nn.LeakyReLU(),
            nn.Linear(50, 10),
            nn.Softmax(dim=1)
        )
        ...
        self.rnn = RNN(input_size=self.embedding_size + self.nn_map_size,
                        ...
                        last_layer_size=self.hidden_state_sizes + self.nn_map_size,
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        logits, hidden_state = self.rnn(x_tokens, processed_cnn_features, initial_hidden_state, self.attention_layer, self.output_layer,
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, last_layer_size, num_rnn_layers, cell_type='LSTM'):
        """
        ...
        input_size_list.extend([LSTMCell(hidden_state_size, new_input_size) for i in range(num_rnn_layers - 2)])
        new_input_size = last_layer_size 
        last_layer = [LSTMCell(hidden_state_size, new_input_size)]
        input_size_list.extend(last_layer)

        self.cells = nn.ModuleList(input_size_list)

    def forward(self, tokens, processed_cnn_features, initial_hidden_state, attention_layer, output_layer: nn.Linear,
        ...
        logits_sequence = []
        current_hidden_state = initial_hidden_state.to('cuda:0') # [nun_rnn_layers, batch_size, 2*hidden_state_sizes]
        input_tokens = embeddings[:,0,:]  # Should have shape (batch_size, embedding_size)
        max_pool = nn.MaxPool1d(1, self.hidden_state_size)
        max_pooled_processed_cnn_features = torch.squeeze(max_pool(processed_cnn_features))

        for i in range(sequence_length):
            for j in range(self.num_rnn_layers):
                if j == 0:
                    input_first_layer = torch.cat((input_tokens, max_pooled_processed_cnn_features), dim=1)
                    output = self.cells[0](input_first_layer, current_hidden_state[0,:,:].clone())
                    current_hidden_state[0,:,:] = torch.unsqueeze(output, 0)

                elif j == (self.num_rnn_layers-1): # last layer
                    # split hidden_state and cell_memory
                    attention_out = attention_layer(current_hidden_state[(j-1),:,:])
                    hidden_state, cell_memory = torch.split(current_hidden_state[(j-1),:,:], self.hidden_state_size , dim=1)
                    
                    attention_mul_sum_cnn_features = torch.einsum('bji,bi->bj',processed_cnn_features, attention_out)
                    
                    last_layer_input = torch.cat((attention_mul_sum_cnn_features, hidden_state), dim=1)
                    output = self.cells[j](last_layer_input.clone(), current_hidden_state[j,:,:].clone())
